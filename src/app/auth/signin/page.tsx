...
function SignInForm() {
  ...
  const [successUser, setSuccessUser] = useState<{ name: string; pending?: boolean } | null>(null);
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profErr || !profile) {
        ...
        profile = newProf;
        // Do not return here, let the code flow to the email_verified check below!
      }

      if (!profile) return; // Fallback for TypeScript
      ...
      if (profile.role === "organizer" && (profile.organizer_status === "pending" || !(profile as any).organizer_status)) {
        setSuccessUser({ name: profile.full_name?.split(" ")[0] || "User", pending: true });
        router.push("/dashboard/organizer/pending");
        return;
      }

      // Success — route based on role
      setSuccessUser({ name: profile.full_name?.split(" ")[0] || "User" });
    ...
    return (
      <div className="glass rounded-2xl p-8 text-center animate-slide-up">
        ...
        <h2 className="font-display text-2xl font-bold mb-2">
          Welcome{successUser.pending ? "" : " back"}, <span className="gradient-text">{successUser.name}</span>! 👋
        </h2>
        <p className="text-muted text-sm mb-2 leading-relaxed">
          {successUser.pending ? "Your application is under review." : "We're setting up your dashboard..."}
        </p>
        <p className="text-xs text-muted/60 animate-pulse">Navigating shortly</p>
      </div>
    );
  }
