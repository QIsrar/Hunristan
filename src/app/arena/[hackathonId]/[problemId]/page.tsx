...
type TabType = "problem" | "submissions" | "discussion" | "ai";
...
interface SubmitResult {
  ...
  ai?: { feedback?: string; score?: number; quality?: number; timeComplexity?: string; spaceComplexity?: string; verdict?: string; suggestions?: string[]; gradedBy?: string; };
  error?: string;
}
...
export default function ArenaPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // ── Registration guard (non-practice only) ──────────────────────
      // Check if user has role participant before enforcing
      ...
      if (!isAdminOrOrganizer) {
        ...
        if (!registration) {
          toast.error("You must register for this hackathon before entering the arena.");
          return router.push(`/hackathons/${hackathonId}`);
        }

        if (registration.payment_status === "pending") {
          toast.error("Your payment is awaiting verification. You'll get access once approved.");
          return router.push(`/hackathons/${hackathonId}`);
        }

        if (registration.payment_status === "rejected") {
          toast.error("Your payment was rejected. Please re-upload your payment screenshot.");
          return router.push(`/hackathons/${hackathonId}`);
        }
      }
      // ───────────────────────────────────────────────────────────────
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (data.verdict === "accepted") {
        ...
        setTab("ai"); // Auto-switch to AI tab on non-accepted result
      }
      // Always show AI tab if we got feedback
      if (data.ai?.feedback) setTab("ai");
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-[46%] flex flex-col border-r border-border overflow-hidden">
          <div className="flex border-b border-border bg-surface/50 shrink-0">
            {(["problem", "ai", "submissions", "discussion"] as TabType[]).map(t => (
              <button key={t} onClick={() => setTab(t)}
                ...
                {t === "problem" ? <BookOpen size={12} /> : t === "submissions" ? <Code2 size={12} /> : t === "discussion" ? <MessageSquare size={12} /> : <Brain size={12} />}
                {t === "ai" ? "AI Review" : t}
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h2 className="font-display font-bold text-lg flex items-center gap-2">
                    <Brain className="text-accent2" size={20} /> AI Code Review
                  </h2>
                  <span className={`text-[10px] px-2 py-1 rounded-full ${submitResult?.ai?.gradedBy === "custom_model" ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-white/5 text-muted border border-white/10"}`}>
                    {submitResult?.ai?.gradedBy === "custom_model" ? "Llama AI Connected" : "Local Fallback Mode"}
                  </span>
                </div>

                {!submitResult?.ai?.feedback ? (
                  <div className="text-center py-20 bg-surface/30 rounded-2xl border border-dashed border-border">
                    <Code2 size={40} className="mx-auto mb-4 text-muted/20" />
                    <p className="text-sm text-muted">Submit your code to see AI insights</p>
                  </div>
                ) : (
                  <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-surface border border-border rounded-xl p-4">
                        <div className="text-[10px] text-muted uppercase tracking-wider mb-1 font-bold">AI Quality Score</div>
                        <div className="text-3xl font-display font-bold text-accent">{submitResult.ai.score}/100</div>
                      </div>
                      <div className="bg-surface border border-border rounded-xl p-4">
                        <div className="text-[10px] text-muted uppercase tracking-wider mb-1 font-bold">Category</div>
                        <div className="text-sm font-bold text-accent2 uppercase mt-2">{submitResult.ai.verdict}</div>
                      </div>
                      <div className="bg-surface border border-border rounded-xl p-4">
                        <div className="text-[10px] text-muted uppercase tracking-wider mb-1 font-bold">Time Complexity</div>
                        <div className="text-sm font-mono text-green-400 mt-2">{submitResult.ai.timeComplexity}</div>
                      </div>
                      <div className="bg-surface border border-border rounded-xl p-4">
                        <div className="text-[10px] text-muted uppercase tracking-wider mb-1 font-bold">Space Complexity</div>
                        <div className="text-sm font-mono text-green-400 mt-2">{submitResult.ai.spaceComplexity}</div>
                      </div>
                    </div>

                    <div className="bg-accent/5 border border-accent/10 rounded-2xl p-5 relative overflow-hidden">
                      ...
                      <h4 className="text-xs font-bold text-accent uppercase tracking-widest mb-3 flex items-center gap-2">
                        <MessageSquare size={14} /> Expert Analysis
                      </h4>
                      <p className="text-sm text-text leading-relaxed italic relative z-10">
                        &quot;{submitResult.ai.feedback}&quot;
                      </p>
                    </div>

                    {(submitResult.ai.suggestions?.length ?? 0) > 0 && (
                      <div className="space-y-3">
                        <h4 className="text-xs font-bold text-muted uppercase tracking-widest">Recommendations</h4>
                        <div className="space-y-2">
                          {(submitResult.ai.suggestions ?? []).map((s, i) => (
                            <div key={i} className="flex gap-3 bg-surface/50 border border-border p-3 rounded-xl text-sm text-muted">
