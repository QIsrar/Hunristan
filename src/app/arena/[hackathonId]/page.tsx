...
export default function ArenaLobbyPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: hack }, { data: probs }, { data: cats }] = await Promise.all([
        ...
        supabase.from("competition_categories").select("id, name, type, max_score, order_index")
          .eq("hackathon_id", hackathonId).order("order_index"),
      ]);
      ...
      if (problemList.length === 0 && categoryList.length === 0) {
  ...
  const handleEnter = async () => {
    if (!agreed || (problems.length === 0 && categories.length === 0)) return;
    setEntering(true);
    if (problems.length > 0) {
      router.push(`/arena/${hackathonId}/${problems[0].id}`);
    } else {
      router.push(`/arena/${hackathonId}/category/${categories[0].id}`);
    }
  };
  ...
  const totalPoints = problems.reduce((sum: any, p: any) => sum + (p.points || 0), 0) + 
                      categories.reduce((sum: any, c: any) => sum + (c.max_score || 0), 0);
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          <div className="glass rounded-xl p-4 text-center">
            ...
            <div className="text-2xl font-bold font-display">{problems.length + categories.length}</div>
        ...
        <div className="glass rounded-xl p-5 mb-5">
          <h3 className="font-semibold text-sm mb-4 flex items-center gap-2">
            <Code2 size={15} className="text-accent" /> Challenge Set
          </h3>
          <div className="space-y-2">
            ...
            {categories.map((c: any, i: number) => (
              <div key={c.id} className="flex items-center justify-between py-2 border-b border-border/50 last:border-0">
                <div className="flex items-center gap-3">
                  <span className="text-xs text-muted w-5">{i + 1 + problems.length}.</span>
                  <span className="text-sm font-medium">{c.name}</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className={`text-[10px] px-2 py-0.5 rounded-full border border-accent/20 bg-accent/10 text-accent uppercase`}>
                    {c.type}
                  </span>
                  <span className="text-xs font-mono text-accent">{c.max_score}pts</span>
                </div>
              </div>
            ))}
          </div>
