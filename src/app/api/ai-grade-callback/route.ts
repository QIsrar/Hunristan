...
export async function POST(request: NextRequest) {
  ...
  // Secret is mandatory — if not configured, block all requests to prevent
  // unauthenticated AI score injection.
  if (!secret) {
    return NextResponse.json(
      { error: "Webhook not configured. Set WEBHOOK_SECRET in environment variables." },
      { status: 503 }
    );
  }
