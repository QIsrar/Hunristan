...
export default function MentorsPage() {
  ...
  useEffect(() => {
    safeGetUser().then(u => {
      if (u) {
        setIsLoggedIn(true);
        supabase.from("profiles").select("email, full_name").eq("id", u.id).single().then(({ data }) => {
          if (data) setForm(f => ({ ...f, email: data.email, full_name: data.full_name || "" }));
        });
      }
    });
    ...
    Promise.all([
      ...
      const statsMap = Object.fromEntries((statsData || []).map((s: any) => [s.mentor_id, s]));
      ...
      <div className="pt-24 pb-16 px-6 max-w-7xl mx-auto">
        ...
        <div className="mt-16 glass rounded-2xl p-10 text-center">
          ...
          {submitted ? (
            ...
            <Link href="/auth/signup?redirect=/mentors" className="btn-primary inline-flex items-center gap-2">
              <Star size={16} /> Create Account to Apply
            </Link>
          ) : (
