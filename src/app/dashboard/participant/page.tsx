...
import {
  ...
  ChevronUp, ChevronDown, Minus, Layers, Cpu, FileText, Image,
  FileUp, CheckSquare, Link2, Users, MessageSquare,
} from "lucide-react";
...
export default function ParticipantDashboard() {
  ...
  const [multiTrackSubs, setMultiTrackSubs] = useState<any[]>([]);
  ...
  useEffect(() => {
    async function load() {
      ...
      // Multi-track submissions for this participant
      const { data: v2 } = await supabase
        .from("submissions_v2")
        .select("*, competition_categories(name, type, max_score), hackathons(title, status, end_time)")
        .eq("participant_id", user.id)
        .order("submitted_at", { ascending: false });
      setMultiTrackSubs(v2 || []);
  ...
  return (
    <div className="min-h-screen bg-bg grid-bg">
      ...
      <div className="pt-20 px-4 md:px-6 pb-12 max-w-7xl mx-auto">
        ...
        {multiTrackSubs.length > 0 && (() => {
          const CAT_ICONS: Record<string, React.ElementType> = { TEXT: FileText, IMAGE: Image, FILE: FileUp, MCQ: CheckSquare, URL: Link2, CODE: Code2 };
          const CAT_COLORS: Record<string, string> = { TEXT: "text-violet-400", IMAGE: "text-pink-400", FILE: "text-amber-400", MCQ: "text-green-400", URL: "text-orange-400", CODE: "text-cyan-400" };
          return (
            <div className="glass rounded-2xl p-6 mb-6">
              <div className="flex items-center justify-between mb-5">
                <h2 className="font-display font-semibold flex items-center gap-2">
                  <Layers size={18} className="text-accent" /> Multi-Track Submissions
                </h2>
                <span className="text-xs text-muted">{multiTrackSubs.length} submission{multiTrackSubs.length !== 1 ? "s" : ""}</span>
              </div>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {(showAllSubs ? multiTrackSubs : multiTrackSubs.slice(0, 6)).map((sub: any) => {
                  const type = sub.competition_categories?.type ?? "TEXT";
                  const CatIcon = CAT_ICONS[type] ?? FileText;
                  const catColor = CAT_COLORS[type] ?? "text-muted";
                  const score = sub.final_score ?? sub.ai_score;
                  const maxScore = sub.competition_categories?.max_score ?? 100;
                  const hackStatus = sub.hackathons?.status;
                  return (
                    <Link key={sub.id}
                      href={`/arena/${sub.hackathon_id}/category/${sub.category_id}`}
                      className="flex flex-col p-4 bg-surface/30 rounded-xl border border-border hover:border-accent/30 transition-all group">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-lg bg-surface flex items-center justify-center border border-border">
                            <CatIcon size={13} className={catColor} />
                          </div>
                          <span className={`text-xs font-medium ${catColor}`}>{type}</span>
                        </div>
                        {sub.ai_status === "DONE"
                          ? <CheckCircle2 size={13} className="text-green-400" />
                          : sub.ai_status === "PROCESSING" || sub.ai_status === "PENDING"
                            ? <Cpu size={13} className="text-accent animate-pulse" />
                            : sub.ai_status === "FAILED"
                              ? <XCircle size={13} className="text-red-400" />
                              : null}
                      </div>
                      <p className="text-xs font-semibold truncate mb-1">{sub.competition_categories?.name ?? "Category"}</p>
                      <p className="text-xs text-muted truncate mb-3">{sub.hackathons?.title ?? "Event"}</p>
                      <div className="mt-auto flex items-center justify-between text-xs">
                        {score !== null && score !== undefined
                          ? <span className="font-mono font-bold text-accent">{score}<span className="text-muted font-normal">/{maxScore}</span></span>
                          : <span className="text-muted/50">Awaiting score</span>}
                        {hackStatus === "active"
                          ? <span className="text-accent group-hover:underline">Enter →</span>
              ...
              {multiTrackSubs.length > 6 && (
                <button 
                  onClick={() => setShowAllSubs(!showAllSubs)}
                  className="w-full text-xs text-accent hover:text-accent/80 text-center mt-4 transition-colors font-medium">
                  {showAllSubs ? "Show less" : `+${multiTrackSubs.length - 6} more submissions`}
                </button>
              )}
            </div>
          );
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="glass rounded-2xl p-6">
              ...
              <div className="space-y-3">
                {leaderboard.map((entry: any, i: number) => (
                  <Link key={entry.id} href={`/users/${entry.user_id}`} className="flex items-center gap-3 hover:bg-white/5 rounded-lg p-1 -mx-1 transition-colors group">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm truncate group-hover:text-accent transition-colors">{(entry as any).profiles?.full_name || "Unknown"}</div>
            ...
            <div className="glass rounded-2xl p-6 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-accent2/5 rounded-full blur-3xl pointer-events-none" />
              <SectionHeader title="Find Teammates" icon={Users} color="text-accent2" />
              <p className="text-xs text-muted mb-4 leading-relaxed">
                Browse participants, check their stats and skills, and team up for the next hackathon.
              </p>
              <Link href="/participants"
                className="btn-secondary w-full text-center flex items-center justify-center gap-2 !py-2">
                <Users size={14}/> Browse Participants
              </Link>
              <Link href="/community"
                className="mt-2 w-full flex items-center justify-center gap-2 px-4 py-2 glass rounded-xl text-sm text-muted hover:text-accent transition-colors">
                <MessageSquare size={14}/> Post in Community
              </Link>
            </div>
