...
import {
  ...
  Loader2, CreditCard, XCircle, ExternalLink, Layers, Scale, Cpu, AlertCircle,
} from "lucide-react";
...
export default function OrganizerDashboard() {
  ...
  const [multiTrackStats, setMultiTrackStats] = useState({ total: 0, aiDone: 0, pendingHuman: 0, failed: 0 });
  ...
  const fetchData = useCallback(async () => {
    ...
    // Fetch multi-track submission stats
    if (hacks && hacks.length > 0) {
      const multiTrackHacks = hacks.filter((h: any) => h.competition_type === "MULTI_TRACK").map((h: any) => h.id);
      if (multiTrackHacks.length > 0) {
        const { data: v2subs } = await supabase
          .from("submissions_v2")
          .select("ai_status, human_score")
          .in("hackathon_id", multiTrackHacks);
        const v2 = v2subs || [];
        setMultiTrackStats({
          total: v2.length,
          aiDone: v2.filter((s: any) => s.ai_status === "DONE").length,
          pendingHuman: v2.filter((s: any) => s.ai_status === "DONE" && s.human_score === null).length,
          failed: v2.filter((s: any) => s.ai_status === "FAILED").length,
        });
      }
    }
  ...
  return (
    <div className="min-h-screen bg-bg grid-bg">
      ...
      <div className="pt-20 px-4 md:px-6 pb-12 max-w-7xl mx-auto">
        ...
        {/* Multi-Track Events Panel */}
        {hackathons.some((h: any) => h.competition_type === "MULTI_TRACK") && (
          <div className="glass rounded-2xl p-6 mb-8">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-display font-semibold flex items-center gap-2">
                <Layers size={16} className="text-accent" /> Multi-Track Events
              </h2>
              <Link href="/dashboard/judge"
                className="flex items-center gap-2 btn-secondary !py-2 !px-4 text-sm">
                <Scale size={13} /> Open Judge Dashboard
                {multiTrackStats.pendingHuman > 0 && (
                  <span className="w-5 h-5 rounded-full bg-amber-500 text-bg text-xs font-bold flex items-center justify-center">
                    {multiTrackStats.pendingHuman}
                  </span>
                )}
              </Link>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
              {[
                { label: "Submissions", value: multiTrackStats.total, color: "text-accent", icon: Layers },
                { label: "AI Scored", value: multiTrackStats.aiDone, color: "text-cyan-400", icon: Cpu },
                { label: "Need Review", value: multiTrackStats.pendingHuman, color: "text-amber-400", icon: Clock },
                { label: "AI Failed", value: multiTrackStats.failed, color: "text-red-400", icon: AlertCircle },
              ].map(s => (
                <div key={s.label} className="bg-surface/50 rounded-xl p-3 border border-border/50">
                  <div className="flex items-center gap-1.5 mb-1"><s.icon size={12} className={s.color} /><span className="text-xs text-muted">{s.label}</span></div>
                  <div className={`font-display text-2xl font-bold ${s.color}`}>{s.value}</div>
                </div>
              ))}
            </div>
            <div className="space-y-2">
              {hackathons.filter((h: any) => h.competition_type === "MULTI_TRACK").map((h: any) => (
                <div key={h.id} className="flex items-center justify-between p-3 bg-surface/30 rounded-xl border border-border/50 text-sm">
                  <div className="flex items-center gap-2">
                    <Layers size={13} className="text-accent" />
                    <span className="font-medium truncate">{h.title}</span>
                    <span className={`text-xs px-1.5 py-0.5 rounded-md badge-${h.status}`}>{h.status}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted">{h.participant_count} registered</span>
                    <Link href="/dashboard/judge" className="text-xs text-accent hover:underline flex items-center gap-1">
                      <Scale size={11} /> Judge
                    </Link>
                  </div>
                </div>
                ...
                <tbody className="divide-y divide-border/50">
                  {hackathons.map((h: any) => (
                    <tr key={h.id} className="hover:bg-white/2 transition-colors">
                      ...
                      {/* Status badge — show 'pending' when not yet approved (overrides time-based DB status) */}
                      <td className="p-4">
                        <StatusBadge
                          status={
                            h.status === "draft"
