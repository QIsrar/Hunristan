...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!prof || prof.role !== "admin") {
        router.push(`/dashboard/${prof?.role || "participant"}`);
        return;
      }
      setProfile(prof);
      try {
  ...
  const postAnnouncement = async () => {
    ...
    const { data: allUsers } = await supabase
      ...
      .select("id")
      .eq("is_banned", false)
      .neq("id", profile.id); // don't notify self
    ...
    toast.success(`Announcement posted to ${allUsers?.length ?? 0} users! 🎉`);
    setPosting(false);
  };
                    ...
                    <tr key={u.id} className={`hover:bg-white/2 transition-colors ${u.is_banned ? "opacity-40" : ""}`}>
                      ...
                      <td className="p-4">
                        <span className={`text-xs px-2 py-1 rounded-full border ${
                          ...
                          {u.role.charAt(0).toUpperCase() + u.role.slice(1)}{u.id === profile?.id ? " (you)" : ""}
                        </span>
                      </td>
            ...
            {hackathons.map(h => (
              <div key={h.id} className="glass rounded-2xl overflow-hidden">
                <div className="flex items-center gap-4 p-4 cursor-pointer hover:bg-white/2 transition-colors"
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full badge-${h.is_approved ? h.status : "pending"} shrink-0`}>
                    {h.is_approved ? h.status : "pending"}
                  </span>
                      ...
                      <div className="flex items-start justify-between gap-4 flex-wrap">
                        <div className="flex items-start gap-4 flex-1 min-w-0">
                          ...
                          <div className="space-y-1 min-w-0">
                            ...
                            <div className="text-xs text-accent3">{app.applicant_role} @ {app.organization}</div>
