...
import {
  ...
  Loader2, Check, Info, Save, ArrowLeft, SendHorizonal,
} from "lucide-react";
...
export default function EditHackathon() {
  ...
  // Competition type (determines step 2 content)
  const [competitionType, setCompetitionType] = useState<"CODING" | "MULTI_TRACK">("CODING");
  const STEPS = ["Basic Info", competitionType === "CODING" ? "Problems" : "Categories", "Rules", "Review & Save"];
  ...
  useEffect(() => {
    async function load() {
      ...
      const cType = (h.competition_type as "CODING" | "MULTI_TRACK") || "CODING";
      setCompetitionType(cType);
      ...
      // Load step 2 data based on competition type
      if (cType === "MULTI_TRACK") {
        ...
        if (cats && cats.length > 0) {
          const drafts: CategoryDraft[] = cats.map((c, idx) => ({
            name: c.name,
            type: c.type,
            description: c.description || "",
            time_limit: c.time_limit?.toString() || "",
            max_score: c.max_score ?? 100,
            max_submissions: (c as any).max_submissions ?? 1,
            allow_resubmit: (c as any).allow_resubmit ?? true,
            order_index: c.order_index ?? idx,
            rubric_json: Array.isArray(c.rubric_json) ? c.rubric_json : [],
            mcq_questions: [],
          }));
  ...
  const validateStep = (s: number): boolean => {
    ...
    if (s === 1) {
      // MULTI_TRACK: validate categories
      if (competitionType === "MULTI_TRACK") {
        if (categories.length === 0) { toast.error("Add at least one category"); return false; }
        for (const cat of categories) {
          if (!cat.name.trim()) { toast.error("Every category needs a name"); return false; }
          if (cat.type === "MCQ" && (!cat.time_limit || parseInt(cat.time_limit) <= 0)) {
            toast.error(`MCQ category "${cat.name}" requires a time limit`); return false;
          }
        }
        return true;
      }
      // CODING: validate problems
  ...
  // mode: 'draft' | 'save' | 'submit'
  const handleSave = async (mode: "draft" | "save" | "submit" = "save") => {
    setSaving(true);
    try {
      // Build status update payload
      let statusPayload: Record<string, unknown> = {};
      if (mode === "draft") statusPayload = { status: "draft" };
      if (mode === "submit") statusPayload = { status: "upcoming", is_approved: false };

      // Update hackathon
      const { error: hErr } = await supabase.from("hackathons").update({
        title, description,
      ...
      // Handle step 2 data based on competition type
      if (competitionType === "MULTI_TRACK") {
        // Delete and re-insert all categories for this hackathon.
        // This is safe because we reload them on mount, and categories without
        // submissions are low-risk to recreate.
        await supabase.from("competition_categories").delete().eq("hackathon_id", id);
        for (let ci = 0; ci < categories.length; ci++) {
          ...
          const { data: savedCat, error: catErr } = await supabase
            .from("competition_categories").insert({
              ...
              max_score: cat.max_score || 100,
              time_limit: cat.time_limit ? parseInt(cat.time_limit) : null,
              order_index: ci,
              is_active: true,
            }).select().single();
          if (catErr) throw new Error(`Category "${cat.name}" save failed: ${catErr.message}`);
          // Save MCQ questions if any
          if (cat.type === "MCQ" && cat.mcq_questions && cat.mcq_questions.length > 0) {
      ...
      if (mode === "submit") {
        toast.success("Hackathon submitted for approval! 🎉 An admin will review it shortly.");
      } else {
        toast.success(mode === "draft" ? "Saved as draft!" : "Hackathon updated successfully! ✅");
      }
    ...
    <div className="min-h-screen bg-bg grid-bg">
      ...
      <div className="pt-24 pb-16 px-6 max-w-4xl mx-auto">
            ...
            {STEPS.map((s, i) => (
              <div key={s} className="flex items-center gap-2">
                <button
                  ...
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${isApproved && (i === 1 || i === 2) ? "glass text-muted/30 cursor-not-allowed"
        ...
        <div className="glass rounded-2xl p-8">
          ...
          {/* ── Step 2: Problems (CODING) or Categories (MULTI_TRACK) ── */}
          {step === 1 && competitionType === "MULTI_TRACK" && (
            <div className="space-y-4">
              <h2 className="font-display text-2xl font-bold">Event Categories</h2>
              <p className="text-sm text-muted">Add and configure the competition tracks for this event.</p>
          ...
          {step === 3 && (
            <div className="space-y-6">
              ...
              {/* Context-aware status notice */}
              {hackathonStatus === "draft" && !isApproved ? (
                <div className="bg-accent/5 border border-accent/20 rounded-xl p-4 text-sm text-muted space-y-1">
                  <p className="text-accent font-semibold text-xs mb-1">📋 Draft Hackathon</p>
                  <p>This hackathon is saved as a <span className="text-text font-medium">draft</span> and is only visible to you.</p>
                  <p className="text-xs mt-1">• <span className="text-text">Save as Draft</span> — keep editing later.</p>
                  <p className="text-xs">• <span className="text-text">Save Changes</span> — update without changing status.</p>
                  <p className="text-xs">• <span className="text-accent font-semibold">Submit for Approval</span> — send to admin for review. Once submitted, it will be listed as <em>upcoming</em> after admin approves.</p>
                </div>
              ) : (
          ...
          <div className="flex justify-between mt-8 pt-6 border-t border-border">
            ...
            {step < STEPS.length - 1 ? (
              ...
              <div className="flex gap-3 flex-wrap justify-end">
                {/* Save as Draft — only for non-approved hackathons */}
                {!isApproved && (
                ...
                <button onClick={() => handleSave("save")} disabled={saving} className="btn-secondary flex items-center gap-2">
                  {saving ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />}
                  Save Changes
                </button>

                {/* Submit for Approval — only shown for drafts not yet approved */}
                {hackathonStatus === "draft" && !isApproved && (
                  <button onClick={() => handleSave("submit")} disabled={saving} className="btn-primary flex items-center gap-2">
                    {saving
